    ...
    def validate(command: list[str]) -> Tuple[bool, Event]:
        ...
        if field_type == "phone":
            ...
            if not address_book.is_phone_unique(new_value):
                raise InvalidArgumentsError(f"Phone number {new_value} вже використовується у іншого користувача.")
            ...
            if not address_book.is_email_unique(new_value):
                raise InvalidArgumentsError(f"Email {new_value} вже використовується у іншого користувача.")
